...
options{2}.options{1}=optionY; % options{2}.options{2}=optionY; % 1st measurments  
options{3}.options{1}=optionY; % options{3}.options{2}=optionY; % 2nd measurments  
...
set(gcf, 'Units', 'centimeters', 'Position', [0 0 21 11], 'PaperUnits', 'centimeters', 'PaperSize', [21 11]); 

dx = .055; dy = .01;  
cols=[7,6,4,9,8, 7,6,4,9,8, 7,6,4,9,8, 7,6,4,9,8];
cols = [12 12 12 11 9]; fits_col=15;  
ii = 1; 
for i=[1 6 11]
    subplotHJ(6,3,ii,dy,dx); grid on; hold on; box on;       
%     yyaxis left % Growth OD600
    ...
    for BRs=1:3
        ...
        for k=1%:size(OD600,2)
             ...
             for n_fits=1:10
                ...
                tps = [linspace(t00,tt_k(end),100)]'; plot(tps/60,log2(f(tps-t00)), '-', 'color', [cmap{fits_col}(BRs,:) .6]);
                TT(n_fits) = mean_T; 
    ...
    if ii==1;ylabel('OD600');end; if logs; ylim([.07 1.1]); set(gca, 'Yscale', 'log'); else; ylim(log2([.07 1.1])); yticks([OD600ticks]); yticklabels({OD600ticksLabels}); end
    ii=ii+1; 
end
...
for i=selects
    ...
    subplotHJ(6,1,1+ii,dy,dx); grid on; hold on; 
    yyaxis left % Growth OD600
    times = (i-1)*5+1; 
    for eee=1:3 
...
%                  if (k==1 & ismember(ii,[5:8])==1); option=optionY; option.howmany=3; end
                 for n_fits=1:10
                    ...
                    tps = [linspace(t00,tt_k(end),100)]'; plot(tps/60,log2(f(tps-t00)), '-', 'color', [cmap{fits_col+kk-1}(BRs,:) .6]);
                    TT(n_fits) = mean_T; 
    ...
    textY=log2([.55 .4]); if ii==3; textY=log2([.30 .2]);end 
    for k=1:3
        if k==1;           
...
figname = [dir_name,'/growth_CSLE_3tp_',num2str(j)];  
print(figname,'-depsc', '-r600');
