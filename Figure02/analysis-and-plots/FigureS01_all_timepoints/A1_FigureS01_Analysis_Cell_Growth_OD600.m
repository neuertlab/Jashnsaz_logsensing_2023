...
options{2}.options{1}=optionN; % options{2}.options{2}=optionN; % 1st measurments  
options{3}.options{1}=optionN; % options{3}.options{2}=optionN; % 2nd measurments  
...
cols = [12 13 13 12 13 13 12 11 9]; fits_col=15;  
ii = 1; 
for i=[1 6 11]
...
figname = [dir_name, '/Growth4','_LOG', num2str(logs), '_mu',num2str(meanBRs), '_S',num2str(smooth_mBRs), '_F', num2str(fullexp)];  
...
figname = [dir_name,'/growth_all_tp_', num2str(j)];  
print(figname,'-depsc', '-r600');

end

save('DATA_FINAL_all_tp','controls','DoublingRates'); 
